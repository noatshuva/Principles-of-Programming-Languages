...
//-------- 3.3 --------
it('Q3.3 - TESTS', () => {
    expect(bind(parseL4('(L4 (define addFunc (lambda (x y) (if (> x y) (+ x y) (/ x 0)))) (addFunc 2 1))'), evalNormalProgram)).to.deep.equal(makeOk(3))
    expect(bind(parseL4('(L4 (define testFunc (lambda (testFun))) (define func (lambda (x y) (if (= y 0) x y))) (func testFunc 1))'), evalNormalProgram)).to.deep.equal(makeOk(1))
    expect(bind(parseL4(`(L4 ((lambda (x y z) (if x y z)) #t 1 (/ 1 0)))`), evalNormalProgram)).to.deep.equal(makeOk(1));
});