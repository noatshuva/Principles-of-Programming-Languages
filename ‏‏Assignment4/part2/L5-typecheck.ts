...
import { isAppExp, isBoolExp, isDefineExp, isIfExp, isLetrecExp, isLetExp, isNumExp,
         ...
         Parsed, PrimOp, ProcExp, Program, StrExp, isValuesExp, isLetValuesExp, LetValuesExp, ValuesExp, VarDecl, CExp } from "./L5-ast";
import { applyTEnv, makeEmptyTEnv, makeExtendTEnv, TEnv } from "./TEnv";
import { isProcTExp, makeBoolTExp, makeNumTExp, makeProcTExp, makeStrTExp, makeVoidTExp,
         parseTE, unparseTExp,
         BoolTExp, NumTExp, StrTExp, TExp, VoidTExp, isTExp, isNonEmptyTupleTExp, isTupleTExp, makeEmptyTupleTExp, isNonTupleTExp, makeNonEmptyTupleTExp } from "./TExp";
...
export const typeofValues = (exp: ValuesExp, tenv: TEnv): Result<TExp> => {
    const varsType = mapResult(e => typeofExp(e, tenv), exp.values);
    return isEmpty(exp.values)? makeOk(makeEmptyTupleTExp()) :
                            bind(varsType, x => allT(isNonTupleTExp, x)? makeOk(makeNonEmptyTupleTExp(x)) : makeFailure("Illegal arguments"));
}
...
export const typeofLetValues = (exp: LetValuesExp, tenv: TEnv): Result<TExp> => {
    ...
    const bodies: CExp[] = map((b) => b.body, exp.bindings);
    const valMap: Result<TExp[]> = (mapResult((b) => typeofExp(b, tenv), bodies));
    const valTexp: TExp[] = makeTExpArray(valMap); 
    if (valTexp.length !== varsMap.length)
        return makeFailure(`The values lengths dont match`);
    const varTEs = map((x: VarDecl) => x.texp, varsMap);
    const constraints = zipWithResult((varTE, val) => bind(makeOk(val),
        (typeOfVal: TExp) => checkEqualType(varTE, typeOfVal, exp)),
        varTEs, valTexp);
    return bind(constraints, _ => typeofExps(exp.body, makeExtendTEnv(map((x: VarDecl) => x.var, varsMap), varTEs, tenv)));
};

const makeTExpArray = (valMap: Result<TExp[]>) : TExp[]=> 
    isOk(valMap)?
        allT(isNonEmptyTupleTExp, valMap.value)?
            reduce((acc: TExp[], curr: TExp) => isNonEmptyTupleTExp(curr) ? reduce((acc2: TExp[], curr2:TExp) => acc2.concat(curr2), acc, curr.TEs) : [curr] , [], valMap.value)
    :[] :[];
