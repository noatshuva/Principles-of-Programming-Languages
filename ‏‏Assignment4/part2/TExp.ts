...
const parseCompoundTExp = (texps: Sexp[]): Result<CompoundTExp> => {
    ...
    return (pos === -1)  ? (texps.length === 1) && (texps[0] === 'Empty') ? makeOk(makeEmptyTupleTExp()) : parseTuple(texps) :
...
const parseTuple = (texps: Sexp[]) : Result<NonEmptyTupleTExp | EmptyTupleTExp> => 
    bind(parseTupleTExp(texps), (texp) => allT(isNonTupleTExp, texp) ? makeOk(makeNonEmptyTupleTExp(texp)) : makeFailure("invalid"));
