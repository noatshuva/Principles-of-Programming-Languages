...
import { join, map, zipWith, is, concat, merge, mergeAll } from "ramda";
...
//         |  ( values <CExp>*)               / ValuesExp(values: CExp[])
//         |  (let-values (<binding2>)* <CExp>+)        / LetValuesExp( bindings: Binding2[], body: CExp[])
// <binding>  ::= ( <var> <cexp> )            / Binding(var:VarDecl, val:Cexp)
// <binding2> ::= ( <var>* <cexp>)            / Binding2(vars:VarDecl[], body:Cexp)
...
export const expComponents = (e: Exp): CExp[] =>
    ...
    isLetValuesExp(e) ? [...e.body, ...map((b) => b.body, e.bindings)] :
...
export interface Binding2 {tag: "Binding2"; vars: VarDecl[]; body: CExp; }
export const makeBinding2 = (v: VarDecl[], body: CExp): Binding2 =>
    ({tag: "Binding2", vars: v, body: body});
...
export const makeValuesExp = (values: CExp[]) : ValuesExp =>
    ({tag: "ValuesExp", values: values});
...
const isGoodBindings2 = (bindings2: Sexp): bindings2 is [Sexp, Sexp][] =>
    isArray(bindings2) && allT(isArray, bindings2);
...
const parseLetValuesExp = (bindings: Sexp, body: Sexp[]): Result<LetValuesExp> =>
    isEmpty(body) ? makeFailure('Body of "let-value" cannot be empty') :
    ! isGoodBindings2(bindings) ? makeFailure(`Invalid bindings2: ${JSON.stringify(bindings)}`) :
...
const parseBindings2 = (bindings: [Sexp, Sexp][]): Result<Binding2[]> =>
    safe2((vds: VarDecl[][], vals: CExp[]) => makeOk(zipWith(makeBinding2, vds, vals)))
        (mapResult((b: Sexp) => isArray(b)? mapResult(parseVarDecl, b): mapResult(parseVarDecl, [b]) ,map(b => b[0], bindings)), mapResult(parseL5CExp, map((b) => b[1], bindings)));
...
const parseValuesExp = (values: Sexp[]): Result<ValuesExp> =>
     bind(mapResult(parseL5CExp, values), (vals: CExp[]) => makeOk(makeValuesExp(vals)));
...
const unparseBindings2 = (bindings: Binding2[]): Result<string> =>
    bind(mapResult(bdg => safe2((vd: string[], val: string) => makeOk(`((${join(" ", vd)}) ${val})`))
        (mapResult((v: VarDecl) => unparseVarDecl(v), bdg.vars), unparse(bdg.body)), bindings),
            (bdgs: string[]) => makeOk(join(" ", bdgs)));
    // safe2((vars: string, val: string) => makeOk(`(${vars} ${val})`))
    // (bind(mapResult(unparseVarDecl, bindings.vars), (arr) => makeOk(`(${arr.join(" ")})`)), unparse(bindings.val));
...
const unparseValuesExp = (val: ValuesExp): Result<string> =>
        bind (mapResult(unparse , val.values), (v: string[]) => makeOk(`(values ${join(" ", v)})`));
