...
import { map, repeat, zipWith, zip, reduce, concat, mapObjIndexed } from "ramda";
import { CExp, Exp, IfExp, LetrecExp, LetExp, ProcExp, Program, SetExp, isCExp, isValuesExp, isLetValuesExp, LetValuesExp, ValuesExp, isVarDecl } from './L5-ast';
...
import { applyEnv, applyEnvBdg, globalEnvAddBinding, makeExtEnv, setFBinding,
         theGlobalEnv, Env, FBinding, ExtEnvVars } from "./L5-env";
import { isClosure, makeClosure, Closure, Value, makeTuple , isValuesSExp, SExpValue, ValuesSExp } from "./L5-value";
import { isEmpty, first, rest, allT } from '../shared/list';
import { Result, makeOk, makeFailure, mapResult, safe2, bind, zipWithResult, isOk } from "../shared/result";
...
const evalValues = (exp: ValuesExp, env: Env): Result<Value> =>
    bind(mapResult((c: CExp) =>  applicativeEval(c, env) ,exp.values), (v: Value[]) =>makeOk(makeTuple(v)));

const evalLetValuesExp = (exp: LetValuesExp, env: Env) : Result<Value> =>{
    const varsMap: VarDecl[] = reduce((acc: VarDecl[], curr: VarDecl[]) => concat(acc, curr) ,[] ,map((b) => b.vars, exp.bindings));
    const bodies: CExp[] = map((b) => b.body, exp.bindings);// [[(f 0)],[(g 5)]]
    const valMap: Result<Value[]> = (mapResult((b) => applicativeEval(b, env), bodies));// cvals = [ 1 2 3 , 4 5 6 ] 
    const valSexp: SExpValue[] = makeValSExpArray(valMap); // [1,2,3,4,5,6]
    if (valSexp.length !== varsMap.length)
        return makeFailure(`The values lengths dont match: valSExp`);
    return evalSequence(exp.body, makeExtEnv(map((v: VarDecl) => v.var  , varsMap) ,valSexp ,env));
}; 


const makeValSExpArray = (valMap: Result<Value[]>) : SExpValue[]=> 
    isOk(valMap)?
        allT(isValuesSExp, valMap.value)?
            reduce((acc: SExpValue[], curr: ValuesSExp) => reduce((acc2: SExpValue[], curr2:SExpValue) => acc2.concat(curr2), acc, curr.val), [], valMap.value) 
    :[] :[];
