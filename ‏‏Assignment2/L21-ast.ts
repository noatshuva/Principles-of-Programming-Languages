...
// <ForExp> -> (for <Exp> <Numexp> <Numexp> <CExp>)
const parseForExp = (vars: Sexp, params: Sexp[]): Result<ForExp> =>
    params.length !== 3 ? makeFailure("Expression not of the form (for <exp> <numexp> <numexp> <cexp>)") :
    isString(vars) && isIdentifier(vars) && !isNumericString(vars) && isArray(params[2]) ?
        (bind(parseL21CExp(params[2]),
        (w:CExp)=>(isString(params[0]) && isNumericString(params[0]) && isString(params[1]) && isNumericString(params[1]) && (+params[0]) <= (+params[1])?
                (((x:VarDecl,y:NumExp,z:NumExp):Result<ForExp>=>(makeOk(makeForExp(x, y, z, w))))(makeVarDecl(vars),makeNumExp(+params[0]),makeNumExp(+params[1])))
